#!/usr/bin/env python3
"""
Pretrain PPO agents for HASO before real experiments.

Uses centralized HASOOrchestrator (not per-node agents) to avoid
overloading data nodes with PPO training overhead.

Saves pretrained agents to pretrainppo/{rounds}/ directory.
Later experiments can load these agents to skip the "cold start" phase
where PPO policy is random and causes high latency.

Usage:
    python pretrain_ppo.py --rounds 200
    python pretrain_ppo.py --rounds 500 --n_nodes 10
"""
...
def pretrain_ppo(
    ...
    """
    Pretrain PPO agents for specified rounds using centralized orchestrator.

    Args:
        n_nodes: Number of nodes in the federation.
        pretrain_rounds: Number of rounds to pretrain (cold-start phase).
        seed: Random seed.
        log_dir: Directory for logs.

    Returns:
        Dict with pretrain stats: elapsed_time, final_loss, final_acc.
    """
    print(f"Pretraining HASO Orchestrator for {pretrain_rounds} rounds...")
    ...
    # Override with centralized orchestrator
    ...
    metrics_history = protocol.run(
        ...
        eval_every=pretrain_rounds,  # Only eval at end to save time
    )

    elapsed = time.perf_counter() - start_time

    # Save pretrained orchestrator
    save_dir = Path(f"pretrainppo/{pretrain_rounds}")
    ensure_dir(str(save_dir))
    orchestrator.save(str(save_dir / "orchestrator.zip"))
    print(f"Saved pretrained orchestrator to: {save_dir}")
    ...
    print(f"Saved pretrain metrics to: {metrics_path}")
    ...
    if final_metrics:
        ...
        print(f"  Mean Latency: {final_metrics.round_latency:.2f}s")
    print(f"  Orchestrator dir: {save_dir}")
    print("=" * 50)
...
def analyze_pretrain_breakeven(
    ...
    """
    Analyze at what round PPO starts reducing latency vs NoHASO baseline.

    Also computes breakeven: after how many experiment rounds does time spent
    pretraining pay off vs using NoHASO from the start.

    Args:
        pretrain_dir: Directory containing pretrain metrics.
        baseline_latency: NoHASO average latency (137s from E1 results).
        nohaso_rounds: Number of rounds in the actual experiment (default: 50).
    """
    import os
    ...
    for rounds_dir in sorted(os.listdir(pretrain_dir)):
        ...
        # Compute breakeven: how many experiment rounds to recoup pretrain cost?
        if is_faster:
    ...
    # Show per-round latency progression
...
def main():
    parser = argparse.ArgumentParser(
        description="Pretrain PPO agents for ChainFSL HASO",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Pretrain for 200 rounds
  python pretrain_ppo.py --rounds 200

  # Pretrain for 500 rounds
  python pretrain_ppo.py --rounds 500

  # Analyze breakeven point
  python pretrain_ppo.py --analyze

  # Pretrain with more nodes
  python pretrain_ppo.py --rounds 300 --n_nodes 20
        """,
    )
    parser.add_argument(
        "--rounds",
    ...
    parser.add_argument(
        "--analyze",
        action="store_true",
        help="Analyze breakeven point instead of training",
    )
